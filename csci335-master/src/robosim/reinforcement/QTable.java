...
public class QTable {
    ...
    public double getLearningRate(int state, int action) {
        double num = 1.0;
        double denom = 1;
        double pair = getQ(state, action);
        for (int i = 0; i < q.length; i ++){
            for (int j = 0; j < q.length; j ++)
                if(q[i][j] == pair){
                    denom += visits[i][j]/rateConstant;
                }
        }
        return num/denom;
    }
    ...
    public int leastVisitedAction(int state) {
        double minv = visits[state][0];
        int mina = 0;
        for (int i = 0; i < visits[state].length; i ++){
            if (visits[state][i] < minv){
                minv = visits[state][i];
                mina = i;
            }
        }
    ...
    public int senseActLearn(int newState, double reward) {
        int newaction;
        int lasta = getLastAction();
        int lasts = getLastState();
        double update = (1-getLearningRate(lasts,lasta)) *
                getQ(lasts, lasta) + getLearningRate(lasts,lasta) *
